 ...
 * for a given postcode coordinate.
 *
 * Current phases: flood risk, water quality (WFD).
 * Future phases will add: riverLevels, bathingWater.
 *
...
(function () {
    ...
    function fetchAll(lat, lon) {
        ...
        var waterQuality = window.SEPA.waterQuality;
        ...
        return Promise.all([
            ...
            waterQuality
                ? waterQuality.queryWaterQuality(lat, lon).catch(function (err) {
                    return { available: false, error: err.message };
                })
                : Promise.resolve({ available: false, error: "Module not loaded" })
